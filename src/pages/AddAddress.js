...
import { saveAddressService } from "../api/user.api";
...
const AddAddress = () => {
    ...
    const [selectedDistrict, setSelectedDistrict] = useState({ id: "", full_name: "" });
    const [selectedWard, setSelectedWard] = useState({ id: "", full_name: "" });
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const { data, error } = await saveAddressService(values);
            if (data) {
    ...
    useEffect(() => {
        if (selectedProvince.id) {
            $.getJSON(`https://esgoo.net/api-tinhthanh/2/${selectedProvince.id}.htm`)
                .then((response) => {
                    if (response.error === 0) {
    ...
    useEffect(() => {
        if (selectedDistrict.id) {
            $.getJSON(`https://esgoo.net/api-tinhthanh/3/${selectedDistrict.id}.htm`)
                .then((response) => {
                    if (response.error === 0) {
    ...
    return (
        <>
            <Meta title="Electronics | Thêm địa chỉ mới" />
            <Container class1="cart-wrapper home-wrapper-2 py-5">
                <div className="row d-flex justify-content-center">
                    <div className="col-9">
                        <div className="d-flex justify-content-between align-items-start">
                            <h3 className="mb-0">Thêm địa chỉ giao hàng mới</h3>
                        </div>
                    </div>
                    <div className="col-9">
                                        ...
                                        <div className="mb-3 w-100">
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    formik.setFieldValue("ward", selectedWard);
